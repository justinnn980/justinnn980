...
(주)홈초이스</br>
전국 디지털케이블TV 가입가구에 VOD 전용 프로그램 공급 및 OTT 서비스를 진행하는 국내 유일 사업자</br>
매출액 - 340억 1,850만원</br>
2024.02 ~ 2025.11 (1년 9개월)
...
<table>
  ...
  <tr>
    <th>서비스</th>
    <td>오초이스N 서비스 - LG헬로비전(매출액 1조), 딜라이브(매출액 3500억)</td>
  </tr>
  <tr>
    <th>주요 내용</th>
    <td>
      - Flutter를 사용하여 iOS 및 Android UI를 통합 및 개발 속도 향상<br>
      - Flutter의 DIO 패키지와 await 기반 비동기 처리 구조로 안정적 API 통신 및 로딩 시간 단축(평균 응답속도 20% 개선)<br>
      - 기존 백엔드 API를 활용하여 마이그레이션 진행으로 개발 속도 향상 및 사용자 경험 지속성 달성<br>
      - 트래픽으로 인한 서버 문제을 방지를 위해 로드밸런싱 가능한 Proxy 서버를 배포하여 안정적인 트래픽 라우팅<br>
      - 로드밸런싱으로 인한 IP 변경 및 방화벽 차단을 방지하기 위해 별도의 EC2 인스턴스를 생성 후 Proxy 서버 배포하여 IP 고정<br>
      - 사용자 분석을 위해 사용자 앱 진입 시 device_info_plus 패키지를 활용해 디바이스 정보 및 MAU 수집<br>
      - Google Analytics(GA) 기반 사용자 트래픽 분석을 통해 인기 콘텐츠 확보 기존대비 MAU 15% 향상<br>
      - SharedPreferences 기반의 페어링 코드 로컬 저장 구조를 구현하여 페어링 프로세스 자동화<br>
      - 전역 접근 가능한 페어링 코드 관리 모듈 설계로 다중 화면 간 상태 일관성 확보<br>
      - 썸네일이 추출 시점을 유동적으로 조정하는 백엔드 API 구현하여 운영 안정성 확보<br>
      - await 기반 비동기 처리로 안정적인 네트워크 응답 흐름 구성<br>
      - 서버 환경을 HTTP → HTTPS로 전환하여 통신 보안 및 사용자 신뢰성 강화<br>
      - (주)도브러너 솔루션 기반 NCG DRM을 Flutter 네이티브 코드에 직접 적용하여 콘텐츠 불법 다운로드 방지율 100% 확보<br>
      - 1년 주기로 성인 인증을 수행하는 백엔드 API 구현하여 고객사 운영 부담 완화<br>
    </td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <th>주요 내용</th>
    <td>
      - 개발팀 리더로 참여하여 기획 요구사항을 기술적으로 검토하고 구현 가능성을 사전에 분석하여 일정 내 안정적인 앱 배포 달성<br>
      - 개발 범위 및 UI/UX 요소를 Figma 프로토타입 기반으로 조율하여 디자인와 개발 간 협업 진행<br>
      - 출석체크 이벤트 영역 백엔드부터 프론트엔드까지 모든 영역을 혼자 구현하였으며, 기획 및 디자인팀의 모든 요구사항을 일정내 소화하여 안정적인 이벤트 시작 및 이벤트 종료 후 사용자 출석 결과 운영팀에 전달<br>
      - GA(Google Analytics) SDK를 직접 연동하고, 이벤트 트래킹 구조를 설계하여 사용자 활동 로그를 실시간 수집,분석 가능한 환경 구축<br>
      - 수집된 로그 데이터를 기반으로 DAU,MAU 지표 시각화 대시보드 구현하여 운영팀의 데이터 리포트 작성 시간 대폭 단축<br>
      - 데이터 기반 기능 개선을 주도하여 평균 DAU 888.6% 상승, 일평균 신규 가입자 수 전월 대비 251.9% 증가<br>
  </td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <th>주요 내용</th>
    <td>
      - 페이지 로딩 지연으로 인한 크롤링 실패 문제를 해결하기 위해 요소 탐색 전 일정 시간 대기 및 재시도 로직 구현하여 롤링 성공률 대폭 향상<br>
      - Docker 기반 서버 환경을 구축하여 크롤러 실행 환경을 컨테이너화함으로써 빠르고 안정적인 배포 구성<br>
      - Selenium의 execute_script를 사용해 페이지 하단까지 스크롤하여 동적 로딩 콘텐츠 전체 수집<br>
      - 순위, 타이틀, 국가, 세부 장르, 회차, 러닝타임, 히트 수 등 7개 주요 메타데이터를 자동 크롤링하여 데이터베이스에 저장 및 활용<br>
      - 전체 크롤링 프로세스를 스케줄러 기반으로 자동화하여 주기적 데이터 갱신 및 최신 콘텐츠 정보 유지<br>
    </td>
  </tr>
</table>
